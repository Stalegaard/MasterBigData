...
public abstract class AlgoritmoGenetico {
	...
	public Individuo ejecuta() {
		for (int i = 0; i < pasos; i++) {
			...
			Cromosoma cRec = recombinar(poblacion.getIndividuo(a1).getCromosoma(), poblacion.getIndividuo(a2).getCromosoma());
			cRec.mutar(probMutacion);
			Individuo ind = new Individuo(cRec, problema);
